...
import com.kitheapp.data.R6PayloadSnapshot
import com.kitheapp.data.R6RewardReceipt
...
class R6ParticipantDeviceTest {
    ...
    @Test
    fun unavailableServerStateKeepsAnHonestRetryAction() {
        var retryRequests = 0
        composeRule.setContent {
            GeoDropTheme {
                R6NearbyContent(
                    loading = false,
                    refreshing = false,
                    error = "Kithe can't reach this Experience right now.",
                    items = emptyList(),
                    selectedDropId = null,
                    unlockingDropId = null,
                    trailProgress = null,
        ...
        composeRule.onNodeWithText("Kithe can't reach this Experience right now.")
            .assertIsDisplayed()
        composeRule.onNodeWithText("Try again").assertIsDisplayed().performClick()
