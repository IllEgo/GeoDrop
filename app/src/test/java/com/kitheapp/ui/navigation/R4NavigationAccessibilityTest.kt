...
import com.kitheapp.data.R9ReportState
...
class R4NavigationAccessibilityTest {
    ...
    @Test
    fun compactNavigationAndExperienceTitleStayOnOneLineAtTwoHundredPercentFontScale() {
        composeRule.setContent {
            CompositionLocalProvider(LocalDensity provides Density(1f, fontScale = 2f)) {
                GeoDropTheme {
                    Column(Modifier.width(320.dp)) {
                        GeoDropExperienceTopBar(
                            experiences = listOf(
                                ExperienceNavigationItem(code = "DEMO2026", isOwned = false)
                            ),
                            activeCode = "DEMO2026",
                            onSelectExperience = {},
                            onJoinExperience = {}
                        ...
                        GeoDropParticipantNavigationBar(
                            selected = ParticipantDestination.NEARBY,
                            onSelect = {}
                        )
                    }
                }
        ...
        val titleHeight = composeRule
            .onNodeWithText("Experience DEMO2026", useUnmergedTree = true)
            .assertIsDisplayed()
            .fetchSemanticsNode()
            .boundsInRoot
            .height
        val collectionHeight = composeRule
        ...
        assertTrue("Experience title wrapped or clipped at 200%", titleHeight <= 52f)
        assertTrue("Collection label wrapped at 200%", collectionHeight <= 40f)
    }
