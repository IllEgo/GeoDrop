...
import com.e3hi.geodrop.ui.components.DropCard
import com.e3hi.geodrop.ui.components.DropVisualState
import com.e3hi.geodrop.ui.components.ResultSheet
import com.e3hi.geodrop.ui.components.ResultSheetState
import com.e3hi.geodrop.ui.components.TrailStrip
import com.e3hi.geodrop.ui.components.UnlockButton
import com.e3hi.geodrop.ui.components.UnlockButtonState
...
fun R6NearbyContent(
    ...
    val browseMode = runCatching { R6BrowseMode.valueOf(browseModeName) }
            ...
            items.isEmpty() -> R6EmptyState(Modifier.align(Alignment.Center))
                ...
                if (!approximateLocationEnabled) {
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(horizontal = GeoDropSpacing.screenGutter, vertical = GeoDropSpacing.sm),
                        ...
                        TextButton(onClick = onRequestLocation) { Text("Turn on") }
                    }
                }
...
@Composable
private fun R6BrowseModeSelector(
    mode: R6BrowseMode,
...
private fun R6DropDetail(
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(GeoDropSpacing.sm)) {
            OutlinedButton(onClick = onReport, modifier = Modifier.weight(1f).height(GeoDropSize.minimumTouchTarget)) {
                Icon(Icons.Rounded.Flag, contentDescription = null)
                Spacer(Modifier.width(4.dp))
                Text("Report")
            }
            OutlinedButton(onClick = onBlock, modifier = Modifier.weight(1f).height(GeoDropSize.minimumTouchTarget)) {
...
@Composable
private fun R6ReportDialog(onDismiss: () -> Unit, onSubmit: (String, String?) -> Unit) {
    val reasons = listOf(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(GeoDropSpacing.xs)) {
                reasons.forEach { (code, label) ->
                    Row(
                        ...
                        TextButton(onClick = { selected = code }) { Text(label) }
                    }
                }
                OutlinedTextField(
                    value = narrative,
...
@Composable
private fun R6ErrorState(message: String, onRetry: () -> Unit, modifier: Modifier = Modifier) {
    Column(
