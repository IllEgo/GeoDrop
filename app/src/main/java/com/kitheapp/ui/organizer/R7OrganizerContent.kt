...
import com.kitheapp.data.R7DropContentKind
import com.kitheapp.data.R7DropDraft
import com.kitheapp.data.R7Experience
import com.kitheapp.data.R7ExperienceDraft
import com.kitheapp.data.R7ExperienceState
import com.kitheapp.data.R7ExpiryMode
import com.kitheapp.data.R7OrganizerAccessState
import com.kitheapp.data.R7OrganizerAccessStatus
import com.kitheapp.data.R7OrganizerDrop
import com.kitheapp.data.R7OrganizerDropSummary
import com.kitheapp.data.R7OrganizerException
import com.kitheapp.data.R7OrganizerGateway
import com.kitheapp.data.R7OrganizerPolicy
import com.kitheapp.ui.theme.GeoDropSize
import com.kitheapp.ui.theme.GeoDropSpacing
import com.kitheapp.util.R5EntryLinks
import com.kitheapp.util.R5EntryParser
import com.kitheapp.util.R5QrCode
...
private fun R7ExperienceDetail(
    ...
    val displayCode = remember(experience.code) { R5EntryParser.displayCode(experience.code) }
    val qrPayload = remember(
        experience.code,
        BuildConfig.APP_LINK_HOST,
        BuildConfig.APP_LINK_CONFIGURED
    ) {
        if (BuildConfig.APP_LINK_CONFIGURED) {
            runCatching {
                R5EntryLinks.qrPayload(BuildConfig.APP_LINK_HOST, experience.code).toString()
            }.getOrNull()
        } else {
            null
        }
    ...
    val qrBitmap = remember(qrPayload) {
        qrPayload?.let { payload -> R5QrCode.createBitmap(payload) }
    }
    val qrShareCard = remember(qrBitmap, experience.name, displayCode) {
        qrBitmap?.let { bitmap ->
            R5QrCode.createShareCard(bitmap, experience.name, displayCode)
        }
    }
    val visibleLink = remember(experience.code, BuildConfig.APP_LINK_HOST) {
        "https://${BuildConfig.APP_LINK_HOST}/e/${experience.code}"
    }
    val shareMessage = remember(experience.name, experience.code, displayCode, qrPayload) {
        if (qrPayload != null) {
            "Join ${experience.name}: $qrPayload\nEvent code: $displayCode"
        } else {
            "Join ${experience.name} with event code $displayCode."
        }
    }
    var qrFeedback by rememberSaveable(experience.code) { mutableStateOf<String?>(null) }
    val saveQrLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.CreateDocument("image/png")
    ) { destination ->
        if (destination != null && qrShareCard != null) {
            qrFeedback = runCatching {
                R5QrCode.writePng(context, destination, qrShareCard)
                "QR image saved."
            }.getOrElse {
                "The QR image could not be saved. Try again."
            }
        }
    }
    Column(Modifier.fillMaxSize()) {
                            ...
                            OutlinedButton(onClick = {
                                if (qrShareCard != null) {
                                    runCatching {
                                        ...
                                        qrFeedback = "The QR image could not be shared. Try again."
                                    }
                                } else {
                                ...
                                Spacer(Modifier.width(GeoDropSpacing.xs))
                                Text(if (qrShareCard != null) "Share QR" else "Share")
                            }
                        }
                        if (qrBitmap != null && qrPayload != null) {
                            HorizontalDivider(Modifier.padding(vertical = GeoDropSpacing.sm))
                            Icon(
                            ...
                            Image(
                                ...
                                contentDescription =
                                    "QR code to join ${experience.name}. Event code $displayCode.",
                                modifier = Modifier.size(240.dp),
                                contentScale = ContentScale.Fit
                            )
                            Text(
                                "Share this QR code and event code with guests. Either one gets them in.",
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(GeoDropSpacing.sm)) {
                                OutlinedButton(onClick = {
                                    clipboard.setText(AnnotatedString(qrPayload))
                                    qrFeedback = "Join link copied."
                                }) {
                                    Icon(Icons.Rounded.ContentCopy, contentDescription = null)
                                    Spacer(Modifier.width(GeoDropSpacing.xs))
                                    Text("Copy link")
                                ...
                                OutlinedButton(onClick = {
                                    saveQrLauncher.launch("kithe-${experience.code}.png")
                                }) {
