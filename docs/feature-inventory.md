...
| Dislikes / downvotes | **D** | A `FirestoreRepo.setDropLike` (dislike path); i `AppViewModel.swift:677` | `drops.dislikeCount`, `drops.dislikedBy` | **Decision B → Deferred.** Remove dislike counts/maps in Phase 2.6; keep the like half. Untangle from the shared `setDropLike` / `DropLikeStatus` code. |
| Complex voting systems | — | (covered by Likes/Dislikes rows above) | — | Direction-doc line resolved by Decision B: keep simple likes, drop dislikes/weighting. |
| Algorithmic recommendations | **D·absent** | — | — | Not implemented. |
| Multiple account types w/ extensive permission matrices | **L (scoped)** | A `UserRole{EXPLORER,BUSINESS}`; group `GroupRole{OWNER,SUBSCRIBER}`; Auth claims `moderator`/`admin`/`suspended` | `users.role` | Two user types (explorer + business) are launch-needed (Phase 2.7). Moderator/admin/suspended are **operational claims**, not a user-facing matrix. Kept; no extended matrix to remove. |
